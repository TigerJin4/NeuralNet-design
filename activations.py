...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        f(z) as described above applied elementwise to `Z`
        """
        e = np.exp(Z - np.amax(Z, axis = 1).reshape((Z.shape[0], 1)))
        
        ### YOUR CODE HERE ###
        # print(e /np.sum(e))
        # print(e/np.sum(e, axis = 1).reshape((Z.shape[0], 1)))
        return e /np.sum(e, axis = 1).reshape((Z.shape[0], 1))
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        e = np.exp(Z - np.amax(Z, axis = 1).reshape((Z.shape[0], 1)))
        sm = e /np.sum(e, axis = 1).reshape((Z.shape[0], 1))
        array = []
        for i in range(sm.shape[0]):
            array.append(np.matmul(dY[i], -np.outer(sm[i], sm[i])+np.diag(sm[i])))
