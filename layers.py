...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.cache: OrderedDict = OrderedDict({"W": 0, "b": 0, "X": 0})# what do you need cache for backprop?
        self.gradients: OrderedDict = OrderedDict({"W": 0, "b": 0})# initialize parameter gradients to zeros
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        Z = self.activation.backward(z, dLdY)
        dX = np.matmul(Z, W.T)
        # W: 3 x 4
        # dLdY: 2 x 4
        # X: 2 x 3
        # b : 1 x 2
        self.gradients["W"] = np.matmul(X.T, Z)

        self.gradients["b"] = np.matmul(Z.T, np.ones((X.shape[0], 1))).T.flatten()
...
class Elman(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.gradients: OrderedDict = OrderedDict({"W": 0, "b": 0, "U": 0})
    ...
    def _init_cache(self, X_shape: Tuple[int]) -> None:
        ...
        self.cache = OrderedDict({"s": [s0], "W": 0, "b": 0, "X": [], "U": 0})
    ...
    def forward_step(self, X: np.ndarray) -> np.ndarray:
        ...
        out = self.activation.forward(np.matmul(X,W) + np.matmul(s[-1], U)+b)
    ...
    def backward(self, dLdY: np.ndarray) -> List[np.ndarray]:
        ...
        for i in range(len(X)-1, -1, -1):
            ...
            dLdY = self.activation.backward(np.matmul(x,W) + np.matmul(s_t1, U) + b, dLdY)
            dLdX.append(np.matmul(dLdY,W.T))
            dLdW.append(np.matmul(x.T, dLdY))
            dLdU.append(np.matmul(s_t1.T, dLdY))
            dLdb.append(np.matmul(dLdY.T, np.ones((dLdY.shape[0], 1))).T.flatten())
            dLdY = np.matmul(dLdY, U.T)

        self.gradients["W"] = np.sum(dLdW, axis=0) 
        self.gradients["b"] = np.sum(dLdb, axis=0) 
        self.gradients["U"] = np.sum(dLdU, axis=0) 
...
class Conv2D(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        W = self.init_weights((kernel_height, kernel_width, self.n_in, self.n_out))# initialize weights using self.init_weights
        b = np.zeros(self.n_out)# initialize biases to zeros

        self.parameters = OrderedDict({"W": W, "b": b})
        self.cache: OrderedDict = OrderedDict({"W": 0, "b": 0, "X": 0, "Z": 0})# what do you need cache for backprop?
        self.gradients: OrderedDict = OrderedDict({"W": 0, "b": 0})# initialize parameter gradients to zeros
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        n = self.n_out
        self.cache["X"] = X
        X, p= pad2d(X, self.pad, kernel_shape, 1)
        ...
        for t in range(n_examples):
            ...
            for n in range(out_channels):
                new_mat = np.zeros((out_rows, out_cols))
                for row in range(out_rows):
                    for col in range(out_cols):
                        for i in range(kernel_height):
                            for j in range(kernel_width):
                                for c in range(in_channels):
                                    new_mat[row, col] += W[i, j, c, n]*X[t, row + i, col + j, c] +b[n]
                batch.append(new_mat)
            Z.append(np.array(batch))

        Z = np.array(Z).transpose(0, 2, 3, 1)
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        X_pad, p= pad2d(X, self.pad, kernel_shape, 1)
        out_rows = int((in_rows + p[0] + p[1] - kernel_height) / self.stride + 1)
        out_cols = int((in_cols + p[2] + p[3] - kernel_width) / self.stride + 1)

        dZ = self.activation.backward(Z, dLdY)
        dW = np.zeros((kernel_height,kernel_width,in_channels, out_channels))
                ...
                for row in range(out_rows):
                    for col in range(out_cols):
                        for i in range(kernel_height):
                            for j in range(kernel_width):
                                for c in range(in_channels):
                                    dW[i,j,c,n] += dZ[t, row, col, n]*X_pad[t, row+i, col+j, c]
                                    dB[n] += dZ[t, row, col, n]
                                    dX[t, row + i, col + j, c] += dZ[t, row, col, n]* W[i,j,c,n]

        self.gradients["W"] = dW
        self.gradients["b"] = dB
        dX = np.array(dX)[:,p[0]:-p[1],p[2]:-p[3],:]
